...
function signinWithUser(user, req, res, onSuccess, stayLoggedIn) {
	...
	req.session.regenerate(function() {
		...
		if (keystone.get('cookie signin') && user.password && stayLoggedIn) {
...
var doSignin = function(lookup, req, res, onSuccess, onFail, stayLoggedIn) {
	var stayLoggedIn = 'undefined' === typeof stayLoggedIn ? true : stayLoggedIn;
		...
		User.model.findOne({ email: emailRegExp }).exec(function(err, user) {
			if (user) {
				user._.password.compare(lookup.password, function(err, isMatch) {
					if (!err && isMatch) {
						postHookedSigninWithUser(user, req, res, onSuccess, onFail, stayLoggedIn);
					} else {
						onFail(err);
					}
				});
