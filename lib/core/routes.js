...
function routes(app) {
	...
	if (this.get('auth') === true) {
		...
		// Visual only implementation of user roles for admin ui. IE. anyone with
		// admin rights cans till go to each list page with a direct path
		// Controled from keystone.js through keystone.set('roles' ...)
		app.all('/keystone*', function (req, res, next) {
			if (keystone.get('roles')) {
				var allowed = _.flatten(_.union(Object.keys(keystone.get('roles')).map(function (key) {
					if (req.user[key]) return keystone.get('roles')[key];
				})));
				_.each(keystone.nav.sections, function (list) {
          list.invisible = !req.user.isSuperUser;
          list.path = null;
          _.each(list.lists, function(section) {
            section.invisible = req.user.isSuperUser ? false : !_.contains(allowed, section.key);
