...
UpdateHandler.prototype.addValidationError = function(path, msg, type) {
	...
	if (this.validationErrors[path]) {
		this.validationErrors[path].type = type || 'required';
	} else {
		this.validationErrors[path] = error;
	}
...
/**
 * Adds a translation for a validationError to the updateHandler
 *
 * @param {string} path that failed validation
 * @param {string} message to display
 * @api public
...
UpdateHandler.prototype.addValidationErrorTranslation = function(path, msg) {
	this.validationErrors[path] = {
		name: 'ValidatorError',
		path: path,
		message: msg
